...
EGO_SUM=(
	...
	"git.sr.ht/~emersion/go-scfg, v0.0.0-20211215104734-c2c7a15d6c99/go.mod"
	"git.sr.ht/~emersion/gqlclient, v0.0.0-20220202181617-4e6e9c763dd2"
	"git.sr.ht/~emersion/gqlclient, v0.0.0-20220202181617-4e6e9c763dd2/go.mod"
	...
	"github.com/juju/ansiterm, v0.0.0-20210929141451-8b71cc96ebdc/go.mod"
	...
	"github.com/mattn/go-colorable, v0.1.10/go.mod"
	...
	"github.com/vektah/gqlparser/v2, v2.2.0/go.mod"
	...
	"golang.org/x/sys, v0.0.0-20220329152356-43be30ef3008/go.mod"
...
src_prepare() {
	...
	# disable completion if flag is unset
	for i in zsh bash fish; do
		use $i || sed -i "/$i/Id" Makefile
	done
	# if none of them are on use, fix line 30
	if ! [ use zsh -o use fish -o use bash ]; then
		sed -i '30s/\\//' Makefile
	fi
}
